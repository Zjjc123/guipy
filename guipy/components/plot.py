...
class Plot(Component):
    ...
    def __init__(self, width, height, xlabel=None, ylabel=None):
        ...
        self.window = pygame.Surface((width-self.yaxis_spacer,height-self.xaxis_spacer))
        self._windrect = self.window.get_rect().inflate(-window_extra,-window_extra)

        self.clear()

    def _x(self,x):
        return translate(x,self.xmin,self.xmax,self._windrect.left,self._windrect.right)
    def _y(self,y):
        return translate(y,self.ymax,self.ymin,self._windrect.top,self._windrect.bottom)
